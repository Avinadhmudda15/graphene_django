...
Project root (where `manage.py` lives), for example:
    ...\graphene_trace_django\graphene_django
...
Open http://127.0.0.1:8000/ (redirects to `/dashboard/` when already logged in, or use `/accounts/login/` to sign in).
...
All roles use the same login page (`/accounts/login/`). After sign-in, `/dashboard/` sends you to the patient, clinician, or admin home according to your `role`.
...
**Patient ↔ clinician demo mapping** is explicit in `seed_data.py` (`PATIENT_CLINICIAN_USERNAME`): **alice** and **bob** → **dr_jones**; **carol**, **david**, **eve** → **dr_patel**. Each run **creates or updates** `PatientProfile` for those usernames so re-seeding fixes missing or wrong assignments.

On Windows, if the terminal errors on Unicode checkmarks in seed output, run:
...
First time setup (use your own path to the folder that contains `manage.py`):

    cd path\to\graphene_django
    git init
...
### Patient feedback  `/dashboard/patient/comments/`
- In the UI the section is labelled **Feedback** (sidebar, page titles, forms); the Django model is still `Comment`.
- Submit text optionally linked to a specific pressure frame; view clinician replies in-thread.
...
- Heatmap + playback + session selector (frame API URL built so `patient_id` stays separate from the frame id in `heatmap.js`)
...
### Analytics (`/analytics/`, `/analytics/patient/…`)
- Same login as all roles; patients see their own data at `/analytics/`; clinicians and admins without a patient in the URL see a patient picker, then a URL of the form `/analytics/patient/<id>/` for the chosen patient.
...
### Reports (`/reports/`, `/reports/patient/…`)
- Same patient-picker behaviour as analytics for clinicians/admins; patients use `/reports/` for their own report; staff URLs use `/reports/patient/<id>/`.
...
| /data/upload/ | Patient (own data) |
| /data/upload/patient/<pk>/ | Clinician / Admin (upload for patient) |
| /django-admin/ | Staff / superuser (Django admin site) |
| /analytics/ | Patient (self); clinician / admin → patient picker |
| /analytics/patient/<pk>/ | Patient (self only); clinician / admin with access |
| /reports/ | Patient (self); clinician / admin → patient picker |
| /reports/patient/<pk>/ | Patient (self only); clinician / admin with access |